    ...
    def calculate_risk_probability(self, predicted_tmin, threshold=0.0):
        ...
        if self.dynamic_sigma is not None:
            if len(self.dynamic_sigma) != len(predicted_tmin):
                raise ValueError("dynamic_sigma length must match number of predictions")
...
class ForecastModelTrainer:
    ...
    def __init__(self, max_depth=4, learning_rate=0.05, n_estimators=150,
...
class BenchmarkSuite:
    ...
    def run_time_series_cv(self, df, base_feature_cols, target_cols, n_splits=4,
                            random_state=42, trainer_kwargs=None, gmm_n_components=3,
                            verbose=True):
        ...
        for train_index, test_index in tscv.split(df):
            ...
            X_test, Y_test = df_test_feat[feature_cols], df_test_feat[target_cols].values
            ...
            risk_engine = FrostRiskEngine(dynamic_sigma=dynamic_sigma, historical_mae_tmin=2.0)
            ...
            if verbose:
                print(f"Fold {fold} | MAE: {mae:.2f}°C | AUC-ROC: {auc:.4f} | LogLoss: {logloss:.4f}")
            fold += 1
    ...
    def plot_reliability_diagram(self, actuals, probs, n_bins=10, save_path=None):
        ...
        prob_true, prob_pred = calibration_curve(actuals, probs, n_bins=n_bins, strategy="quantile")
...
class HyperparameterOptimizer:
    ...
    def __init__(self, df, base_feature_cols, target_col_names, n_splits=4, random_state=42):
        self.df = df
    ...
    def _objective(self, trial):
        ...
        avg_metrics = self.bench.run_time_series_cv(
            self.df, self.base_feature_cols, self.target_col_names,
            n_splits=self.n_splits, random_state=self.random_state,
            trainer_kwargs=trainer_kwargs, gmm_n_components=n_components,
            verbose=False,
        )
    ...
    def run(self, n_trials=60, timeout=None):
        ...
        self.study.optimize(self._objective, n_trials=n_trials, timeout=timeout, show_progress_bar=True)
        return self.study
    ...
    def report(self, output_dir="."):
        ...
        fig3 = opt_mpl.plot_pareto_front(self.study, target_names=["AUC-ROC", "LogLoss"])
...
if __name__ == "__main__":
    ...
    selection = optimizer.select_trial(baseline_auc, baseline_logloss, min_auc_gain=0.01)
    best_trial = selection["trial"]

    print("\n################ RESULT ################")
    print(f"Baseline   -> AUC: {baseline_auc:.4f} | LogLoss: {baseline_logloss:.4f}")
    print(f"Best trial -> AUC: {best_trial.values[0]:.4f} | LogLoss: {best_trial.values[1]:.4f}")
    print(f"Best params: {best_trial.params}")
    print(f"ΔAUC = {selection['auc_gain']:+.4f} | ΔLogLoss = {-selection['logloss_drop']:+.4f}")
