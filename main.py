...
class ExplainabilityEngine:
    ...
    def plot_dependence(
        self, feature, X, shap_values=None, interaction_feature="auto", save_path=None
    ):
    ...
    def explain_single_day(self, X_row, top_n=3):
        ...
        contributions = sorted(
            zip(self.feature_names, row_shap), key=lambda c: abs(c[1]), reverse=True
        )[:top_n]
...
if __name__ == "__main__":
    ...
    final_trainer_kwargs = {
        k: v for k, v in best_trial.params.items() if k != "n_components"
    }
    final_trainer = ForecastModelTrainer(random_state=42, **final_trainer_kwargs)
    final_trainer.train(X_full, Y_full)

    tmin_estimator = final_trainer.model.estimators_[
        target_col_names.index("tmin_next")
    ]
    ...
    day_explanation = explainer_engine.explain_day_with_risk(
        sample_day, risk_engine_final
    )
    ...
    print(
        tabulate(
            [
                [d["feature"], f"{d['shap_value']:+.4f}", d["direction"]]
                for d in day_explanation["top_drivers"]
            ],
            headers=["Feature", "SHAP Value", "Effect"],
            tablefmt="pipe",
        )
    )
